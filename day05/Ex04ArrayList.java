...
public class Ex04ArrayList {
	public static void main(String[] args) {
		...
		// ArrayList는 <>안에 어떤 데이터타입의 모음인지 정해주게 된다.
		// 단, <> 안에는 오직 클래스만 들어올 수 있다.
		// 기본형 데이터아입 8가지는 들어올 수 없으나 정수를 모아놓는 ArrayLIst는
		// 자바가  기본형 데이터타입을 클래스로 바꿔둔 포장클래스(wrapper class)를 적어주면 된다.
		// 예를들어 int의 경우 Integer라는 클래스가 int의 포장클래스가 된다.
		...
		//7. ArrayList에서 특정 index의 요소를 삭제할 때에는 
		...
		// equals() 메소드을 활용하기 때문이다.
		// s44는 비록 list에 추가된 적은 없지만
		// s4와 equals()가 true가 나오는 객체이다.
		Student s44 = new Student(4,"4",4,4,4);
				
				
		// 8. 객체가 해당 list에 존재하는 체크하고 싶다면
		//    contains(객체) 메소드를 실행하면 된다.
		//    이 때에는 굳이 똑같은 객체가 아니더라도
		//    해당객체가 equals()가 true 가 나오는 객체가
		//    리스트에 존재한다면 true가 나온다.
		System.out.println("8. contain(elements)");
		// 우리의 list에서 s3는 remove(index)가 삭제되었으므로
		// 존재하지 않는게 확실
		// list에 추가했던 s2의 경우
		...
		// list에 추가된 적은 없지만 equals()가 true가 나오는 객체가 존재하는 s44경우
		
		System.out.println("list.contains(s44) : "+list.contains(s44));
		System.out.println("---------------------");

		//9. 특정 객체의 앞에서부터 세었을 때의 index가 궁금하다면
		//   indexOF(객체)를 실행하면 된다.
		//   단 해당 객체가 존재하지 않을 경우 -1이 리턴된다.
		System.out.println("9. indexOf(elements)");
		...
		// 10. 만약 뒤에서부터 기준으로 해당 객체가 몇번 index에 존재하는지 알려면
		//     lastIndexOf(객체)를 실행하면 된다.
		//     indexOf()와 마찬가지로
		//     존재하지 않는 객체에 대해서는 -1이 리턴한다.
		list.add(s2);

		System.out.println("10. lastindexOf(elements)");
		...
		// 리스트에 추가된 적은 없지만 equals()가 true가 나오는 요소가 존재하는 s44의 경우
